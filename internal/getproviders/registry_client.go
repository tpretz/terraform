...
func (c *registryClient) LegacyProviderDefaultNamespace(typeName string) (string, string, error) {
	...
	if body.RedirectTo != "" {
		redirect, diags = addrs.ParseProviderSourceString(body.RedirectTo)
		if diags.HasErrors() {
			return "", "", fmt.Errorf("Error parsing provider ID from Registry: %s", diags.Err())
		}

		if redirect.Type != typeName {
			return "", "", fmt.Errorf("Registry returned provider with type %q, expected %q", redirect.Type, typeName)
		}
	}

	return provider.Namespace, redirect.Namespace, nil
}
